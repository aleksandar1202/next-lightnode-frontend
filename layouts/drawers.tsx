  ...
  return (<div className="relative flex overflow-hidden flex-col md:flex-row">
    ...
    <div className="flex flex-col md:hidden relative">
      <div className="flex justify-between py-4 px-6 items-center">
        <Link href="/">
          <a><LNLogo size="small" variant="horizontal"/></a>
        </Link>
        <FontAwesomeIcon className="header-open-drawer" icon={faBars} onClick={() => setVisible(!visible)}/>
      </div>
      <div className={mobileDropdownStyles} style={{height: "calc(var(--vh, 1vh) * 100)"}}>
        <FontAwesomeIcon className="header-close-drawer ml-52 mb-12" icon={faTimes} onClick={() => setVisible(!visible)} />
        <NavBar />
      </div>
    </div>
