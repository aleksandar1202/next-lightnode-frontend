...
export const LNContactUsForm = () => {
  const [form, setForm] = useState<ContactUsFormType>({ name: '', email: '', company: '', subject: '', message: '' })
  ...
  return (<form>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <LNInput size="large" value={form.name} onChange={(v: string) => onChangeField('name', v)} placeHolder="Your name"/>
      <LNInput size="large" value={form.email} onChange={(v: string) => onChangeField('email', v)} placeHolder="Email address"/>
      <LNInput size="large" value={form.company} onChange={(v: string) => onChangeField('company', v)} placeHolder="Your company"/>
      <LNInput size="large" value={form.subject} onChange={(v: string) => onChangeField('subject', v)} placeHolder="Subject"/>
    </div>
