...
export const LNSimpleBlog = ({
  ...
  const ephemaral = useMemo(() => ({
    ...
    before: noBorder ? '' : ` before:content-[""] before:h-1 before:w-full before:absolute before:-top-2 before:bg-slate-700 ${side === 'ltr' ? 'before:rotate-3' : 'before:rotate-357'}`
  }), [side, noBorder])
  ...
  const onMouseLeave = (event: SyntheticEvent) => {
    if (btnRef && btnRef.current) {
      btnRef.current.className = btnRef.current.className.replace(' animated zoomIn', '')
    }
  }
